...
by calling `pi -p` as a subprocess (uses the same auth and provider config as
the current Pi session, no separate API key needed).
"""
...
def _call_pi(prompt: str, model: str | None, timeout: int = 300) -> str:
    ...
    body and can easily exceed comfortable argv length. Tools are disabled
    because this is a pure text-generation call.
    """
    cmd = [
        "pi",
        "-p",
...
The description appears in Pi's available_skills list in the system prompt. When a user sends a query, the model decides whether to invoke the skill based solely on the title and on this description. Your goal is to write a description that triggers for relevant queries, and doesn't trigger for irrelevant ones.
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # one-shot, so we inline the prior output into the new prompt.)
